...
function getmodel(contextdoc){
    ...
    var valueList = contextdoc.split(',');
    valueList.forEach(element => {
        console.log('开始解析' + element);
        
        // 遍历类型配置
        for (const typeitem of typelist) {
            while(element.indexOf(typeitem.name) > -1){
                console.log(element + '|解析匹配到类型：' + typeitem.name);
                // 查找类型名起始位置
                ...
                if(list.findIndex(c => c.name == name) < 0){
                    ...
                    console.log(element + '|解析完成写入：' + data.name + '|' + data.type+'|' + data.note);
                    break;
                } else {
                    break;
                }
